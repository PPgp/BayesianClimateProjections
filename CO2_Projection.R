...
model.ar1.const.results.2110 <- fit.project.model(data.medium, year.start=2005,
                                                  year.end=2110, fit.ar1.const=T)

save(model.ar1.const.results.2110, file=paste0(data.location, "model_results_ar1const_2110.rda"))
load(file=paste0(data.location, "model_results_ar1const_2110.rda"))
...
save(model.ar1.const.results.2010, file=paste0(data.location, "model_results_ar1const_2010.rda"))
...
proj.evals.2110.ar1.const <- evaluate.projections(data.medium, model.ar1.const.results.2110,
                                                  year.start=2005, year.end=2110,
                                                  outofsample.validate=F,
                                                  quantiles=c(0.025, 0.05, 0.5, 0.95, 0.975))
save(proj.evals.2110.ar1.const, file=paste0(data.location, "proj_evals_ar1const_2110.rda"))
load(file=paste0(data.location, "proj_evals_ar1const_2110.rda"))
...
# =============================================================
## Start from USA.
ref_level <- data.medium$CO2.total[(data.medium$Isocode == 'USA') & (data.medium$Year == 2005)] * .74 * 11/3 * 1e6
param.indeces <- round(seq(1, floor(1e5 / 20), length.out=(1000 / 5)))
...
proj.evals.2015.ar1.const.meet <- proj.evals.2015.ar1.const
for (j in 1:1000)
{
  if (proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$CO2[3] < ref_level)
  {
    next
  }
  else
  {
    ratio <- ref_level / proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$CO2[3]
    tech_ratio <- ratio^(1/(1 + corr_level[j]))
    tech_ratio <- tech_ratio^c(0, 0.5, rep(1, 16))
    gdp_ratio <- ratio^(corr_level[j]/(1 + corr_level[j]))
    gdp_ratio <- gdp_ratio^c(0, 0.5, rep(1, 16))
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita * gdp_ratio
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech * tech_ratio
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDP <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Pop * 1e3
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$CO2 <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDP * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech / 1e4
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$CO2percapita <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech / 1e4
  }
}
...
for (j in 1:1000)
{
  ...
  else
  {
    ratio <- ref_level / proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$CO2[3]
    tech_ratio <- ratio^(1/(1 + corr_level[j]))
    tech_ratio <- tech_ratio^seq(0, 8.5, length.out = 18)
    gdp_ratio <- ratio^(corr_level[j]/(1 + corr_level[j]))
    gdp_ratio <- gdp_ratio^seq(0, 8.5, length.out = 18)
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita * gdp_ratio
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$Tech * tech_ratio
...
## China
ref_level <- data.medium$Tech[(data.medium$Isocode == 'CHN') & (data.medium$Year == 2005)] * 0.4
param.indeces <- round(seq(1, floor(1e5 / 20), length.out=(1000 / 5)))
corr_level <- as.numeric(model.ar1.const.results.2015$corr.model.out[[2]][[1]][param.indeces, 'rho'])
for (j in 2:5)
{
...
for (j in 1:1000)
{
  if (proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech[4] < ref_level)
  {
    next
  }
  else
  {
    ratio <- ref_level / proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech[4]
    tech_ratio <- ratio
    tech_ratio <- tech_ratio^c(0, 1/3, 2/3, rep(1, 15))
    gdp_ratio <- ratio^(corr_level[j])
    gdp_ratio <- gdp_ratio^c(0, 1/3, 2/3, rep(1, 15))
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$FrontierGap <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$FrontierGap + log(gdp_ratio)
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech * tech_ratio
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$GDPpercapita <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$USA$GDPpercapita / 
      exp(proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$FrontierGap)
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$GDP <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$GDPpercapita * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Pop * 1e3
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$CO2 <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$GDP * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech / 1e4
    proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$CO2percapita <- 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$GDPpercapita * 
      proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[j]]$CHN$Tech / 1e4
  }
}
...
temp <- proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[1]]
proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[1]] <- list()
proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[1]]$USA <- temp$USA
proj.evals.2015.ar1.const.meet$ipat.components.bycountry[[1]]$CHN <- temp$CHN
proj.evals.2015.ar1.const.meet$ipat.quantiles.bycountry <- get.ipat.quantiles.bycountry(proj.evals.2015.ar1.const.meet$ipat.components.bycountry, 
                                                                                        quantiles = c(0.025, 0.05, 0.5, 0.95, 0.975))

ipat.olddata.rm <- get.ipat.pastdata(data.medium, seq(1960, 2015, 5), c("CHN"),
                                     na.remove=T)
ipat.olddata.rm$Pop <- ipat.olddata.rm$Pop / 1e3
plot.ipat.projections(proj.evals.2015.ar1.const.meet$ipat.quantiles.bycountry$CHN,
                      ipat.olddata.rm)

plot.paristarget('CHN', c(2030, (1-.6) * subset(data.medium, Year == 2005 & Isocode == "CHN")$Tech), indeces = 1:18, 
                 intensity = T, title = "China's Intensity", proj.evals.2010.ar1.const = proj.evals.2015.ar1.const.meet)
plot.paristarget('CHN', c(2030, (1-.6) * subset(data.medium, Year == 2005 & Isocode == "CHN")$Tech), indeces = 1:18, 
                 intensity = T, title = "China's Intensity", proj.evals.2010.ar1.const = proj.evals.2015.ar1.const)

plot.paristarget('USA', c(2025, (1-.26) * base.usa.2005.tmp / 10^9), indeces = 1:18, 
                 intensity = F, newdata = 1.476913 * (11 / 3), title = 'United States Total CO2 Emissions', 
                 proj.evals.2015.ar1.const)
...
plot.co2.projections <- function(data.medium,
  ...
  plot.obj <- ggplot() +
    ...
    scale_x_continuous(breaks=seq(1960,year.end,by=20))
  ...
  trajs.quants.transp$Year <- seq(2010, year.end, by=5)
...
plot.co2 <- plot.co2.projections(data.medium,
                                 proj.evals.2300.ar1.const$trajs.quants)
...
plot.co2.cum <- plot.co2.projections.cum(data.medium,
                                         proj.evals.2300.ar1.const$cum.emissions.quantile, year.end = 2300)
...
pdf(paste0(plot.location, "Empirical_CDF_of_Temperature.pdf"))
plot(ecdf(tempinc.spm5), col = 'red', xlab = 'Temperature Increases', ylab = 'Probability', main = 'Empirical Cumulative Probability')
abline(h = seq(0,1,0.05), col='grey', lwd = 0.5)
...
plot.paristarget <- function(Iso, objective, indeces, intensity = F, newdata = NULL, title, proj.evals.2010.ar1.const=proj.evals.2010.ar1.const)
{
...
plot.USA <- plot.paristarget('USA', c(2025, (1-.26) * base.usa.2005.tmp / 10^9), indeces = 1:4, 
...
pdf(paste0(plot.location, "GDP_per_capita_by_country_new", ".pdf"),
    height=14, width=10)
#par(mfrow = c(5,3), mar = c(4.5, 4.5, 1.5, 1) + 0.5)

isos.toplot <- sapply(countries.toplot, getIso)
ggplots_countries <- list()

for(iso in isos.toplot)
{
  data.toplot.tmp <- data.medium[data.medium$Isocode == iso, c('Year', 'CO2', 'Tech', 'PopTotal', 'GDP')]
  data.toplot.proj.list <- proj.evals.2015.ar1.const$ipat.quantiles.bycountry[[iso]]
  data.toplot.proj <- data.frame(Year = seq(2015, 2100, 5), q025 = data.toplot.proj.list$`0.025`$GDPpercapita, 
  ...
  p <- ggplot(data = data.toplot.proj, aes(x = Year, y = q5)) + 
    geom_line(color = 'red') + scale_y_log10() + theme_bw() + 
    geom_ribbon(aes(ymin = q05, ymax = q95), alpha = 0.3, fill = 'red') +
    geom_ribbon(aes(ymin = q025, ymax = q975), alpha = 0.2, fill = 'red') + 
    ylab('GDP per capita per Year') + ggtitle(names(isos.toplot)[which(isos.toplot == iso)]) + 
    geom_line(data = data.toplot.tmp, aes(x = Year, y = GDP/1000)) +
    theme(axis.text=element_text(size=10), axis.title=element_text(size=10,face="bold"), 
          plot.title = element_text(size=12,face="bold")) 
  
  data.toplot.tmp <- data_medium[data_medium$Isocode == iso, c('Year', 'CO2', 'Tech', 'PopTotal', 'GDP')]
  ...
  data.toplot.proj <- data.frame(Year = seq(2010, 2100, 5), q025 = data.toplot.proj.list$`0.025`$GDPpercapita, 
                                 ...
                                 q95 = data.toplot.proj.list$`0.95`$GDPpercapita, q975 = data.toplot.proj.list$`0.975`$GDPpercapita)
  data.toplot.proj[,-1] <- data.toplot.proj[,-1]/1000
  p <- p + geom_line(data = data.toplot.proj, aes(x = Year, y = q5), color = 'blue') + 
    geom_ribbon(data = data.toplot.proj, aes(x = Year, ymin = q05, ymax = q95), alpha = 0.2, fill = 'blue') +
    geom_line(data = data.toplot.tmp, aes(x = Year, y = GDP/1000), col = 'blue')
  ggplots_countries[[iso]] <- p
  
}
grid.arrange(ggplots_countries[[1]], ggplots_countries[[2]], ggplots_countries[[3]],
             ggplots_countries[[4]], ggplots_countries[[5]], ggplots_countries[[6]],
             ggplots_countries[[7]], ggplots_countries[[8]], ggplots_countries[[9]],
             ggplots_countries[[10]], ggplots_countries[[11]], ggplots_countries[[12]],
             ggplots_countries[[13]], ggplots_countries[[14]], ggplots_countries[[15]],
             ncol = 3)
...
pdf(paste0(plot.location, "Carbon_intensity_by_country_new", ".pdf"),
    height=14, width=10)
...
for(iso in isos.toplot)
{
  data.toplot.tmp <- data.medium[data.medium$Isocode == iso, c('Year', 'CO2', 'Tech', 'PopTotal', 'GDP')]
  data.toplot.proj.list <- proj.evals.2015.ar1.const$ipat.quantiles.bycountry[[iso]]
  data.toplot.proj <- data.frame(Year = seq(2015, 2100, 5), q025 = data.toplot.proj.list$`0.025`$Tech, 
                                 q05 = data.toplot.proj.list$`0.05`$Tech, q5 = data.toplot.proj.list$`0.5`$Tech, 
                                 q95 = data.toplot.proj.list$`0.95`$Tech, q975 = data.toplot.proj.list$`0.975`$Tech)
  ymin <- min(min(data.toplot.tmp$Tech), min(data.toplot.proj[,-1]), na.rm = T)
  ymax <- max(max(data.toplot.tmp$Tech), max(data.toplot.proj[,-1]), na.rm = T)
  # plot(c(1960,2100), c(ymin, ymax), type = 'n', xlab = '', 
  #      ylab = 'GDP per capita', main = names(isos.toplot)[which(isos.toplot == iso)], log = 'y')
  # lines(data.toplot.tmp$Year, data.toplot.tmp$Tech, lwd= 1)
  # lines(data.toplot.proj$Year, data.toplot.proj$q5, col = 'red', lty =1, lwd=2)
  # lines(data.toplot.proj$Year, data.toplot.proj$q025, col = 'red', lty =2, lwd=2)
  # lines(data.toplot.proj$Year, data.toplot.proj$q05, col = 'red', lty =2, lwd=2)
  # lines(data.toplot.proj$Year, data.toplot.proj$q95, col = 'red', lty =2, lwd=2)
  ...
  p <- ggplot(data = data.toplot.proj, aes(x = Year, y = q5)) + 
    geom_line(color = 'red') + scale_y_log10() + theme_bw() + 
    geom_ribbon(aes(ymin = q05, ymax = q95), alpha = 0.2, fill = 'red') +
    ylab('Carbon Intensity per Year') + ggtitle(names(isos.toplot)[which(isos.toplot == iso)]) + 
    geom_line(data = data.toplot.tmp, aes(x = Year, y = Tech)) +
    theme(axis.text=element_text(size=10), axis.title=element_text(size=10,face="bold"), 
          plot.title = element_text(size=12,face="bold")) 
  
  data.toplot.tmp <- data_medium[data_medium$Isocode == iso, c('Year', 'CO2', 'Tech', 'PopTotal', 'GDP')]
  data.toplot.proj.list <- proj.evals.2010.ar1.const$ipat.quantiles.bycountry[[iso]]
  data.toplot.proj <- data.frame(Year = seq(2010, 2100, 5), q025 = data.toplot.proj.list$`0.025`$Tech, 
                                 q05 = data.toplot.proj.list$`0.05`$Tech, q5 = data.toplot.proj.list$`0.5`$Tech, 
                                 q95 = data.toplot.proj.list$`0.95`$Tech, q975 = data.toplot.proj.list$`0.975`$Tech)
  p <- p + geom_line(data = data.toplot.proj, aes(x = Year, y = q5), color = 'blue') + 
    geom_ribbon(data = data.toplot.proj, aes(x = Year, ymin = q05, ymax = q95), alpha = 0.2, fill = 'blue') +
    geom_line(data = data.toplot.tmp, aes(x = Year, y = Tech), col = 'blue')
